        ...
        container.querySelectorAll("div.language-mermaid").forEach((el) => {
            try {
                ...
                if (!parent || !parent.classList.contains("mermaid")) {
                    if (parent) parent.classList.add("mermaid");
                    else el.classList.add("mermaid");
                }
            } catch (e) {}
        });
        try {
            await mermaid.run({ querySelector: ".asri-enhance-sidememo-container .language-mermaid" });
        } catch (e) {}
    } catch (err) {}
}
...
function syncFoldState(item: HTMLElement, sourceEls: HTMLElement[]): void {
    try {
        const shouldFold = sourceEls.some(el => el.hasAttribute?.("asri-enhance-sidememo-fold"));
        if (shouldFold) {
...
function createMemoItem(config: MemoItemConfig): HTMLElement {
    ...
    item.setAttribute("asri-enhance-sidememo-uid-" + config.uid, "");
    item.style.top = `${config.top}px`;
    syncFoldState(item, config.sourceEls);
    return item;
}
...
function syncSidememoState(): void {
    const protyleTitles = Array.from(document.querySelectorAll<HTMLElement>(".protyle-title"));
    ...
    if (isActive) {
        ...
        try {
            const leftover = Array.from(document.querySelectorAll<HTMLElement>(".asri-enhance-sidememo-container"));
            leftover.forEach((c) => { try { c.remove(); } catch (e) {} });
        } catch (e) {}
    }
}
async function refreshSidememoContainers(): Promise<void> {
    const titleElements = Array.from(
        document.querySelectorAll<HTMLElement>(".asri-enhance-sidememo .protyle-top .protyle-title"),
    );
    for (const titleEl of titleElements) {
        try {
            let container = titleEl.querySelector<HTMLElement>(".asri-enhance-sidememo-container");
            if (!container) {
                container = document.createElement("div");
                container.className = "asri-enhance-sidememo-container protyle-custom";
                titleEl.appendChild(container);
            }
            try {
                const protyleTop = titleEl.closest(".protyle-top") as HTMLElement | null;
                if (protyleTop) {
                    if (titleEl.classList.contains("fn__none")) {
                        protyleTop.classList.add("asri-enhance-sidememo-protyle-title-none");
                    } else {
                        protyleTop.classList.remove("asri-enhance-sidememo-protyle-title-none");
                    }
                }
            } catch (e) {}
            try { attachNoRightClick(container); } catch (e) {}
            try {
    ...
    const allContainers = Array.from(
        document.querySelectorAll<HTMLElement>(".protyle-top .protyle-title > .asri-enhance-sidememo-container"),
    );
    allContainers.forEach((container) => {
        ...
        if (!titleParent.closest(".asri-enhance-sidememo")) {
            try {
                const protyleTop = titleParent.closest(".protyle-top") as HTMLElement | null;
                if (protyleTop) {
                    protyleTop.classList.remove("asri-enhance-sidememo-protyle-title-none");
                }
            } catch (e) {}
                        ...
                        if (uid) {
                            ...
                            sources.forEach(source => {
                                if (source && source.setAttribute) {
                                    ...
                                    else { try { source.removeAttribute("asri-enhance-sidememo-fold"); } catch (e) {} }
                                }
                            });
                        }
                    } catch (e) {}
...
async function populateSidememoContainer(container: HTMLElement, protyleContent: HTMLElement): Promise<void> {
    ...
    while (container.firstChild) {
        ...
        try {
            ...
            if (handlers) {
                ...
                try {
                    if (handlers._asriContainer) handlers._asriContainer.classList.remove("asri-enhance-sidememo-container-resize");
                    if (handlers._asriParent) handlers._asriParent.classList.remove("asri-enhance-sidememo-container-resize");
                } catch (e) {}
    ...
    allMemoGroups.forEach((memoGroup, groupIndex) => {
        try {
            ...
            const isBlock = !!memoGroup.el && !memoGroup.el.classList.contains("protyle-wysiwyg");
            const isFile = !!memoGroup.el && memoGroup.el.classList.contains("protyle-wysiwyg");
            ...
            if (isInline) {
                ...
                uid = getUidFromElement(elements[0]) || ensureMemoUid(elements[0]);
                elements.forEach(el => { try { setUidToElement(el, uid); } catch (e) {} });
                top = calcMemoTop(elements[0], container, protyleContent);
                sourceEls = elements;
                type = 'inline';
                item = createMemoItem({
                    className: "asri-enhance-sidememo-inlinememo-item",
                    titleClassName: "asri-enhance-sidememo-inlinememo-item-title",
                    contentClassName: "asri-enhance-sidememo-inlinememo-item-content",
                    titleText,
                    contentText: content,
                    top,
                    uid,
                ...
                const memoEl = memoGroup.el!;
                const titleText = (memoEl.textContent || "").trim();
                const contentText = memoEl.getAttribute("memo") || "";
                uid = ensureMemoUid(memoEl);
                top = calcMemoTop(memoEl, container, protyleContent);
                sourceEls = [memoEl];
                type = 'block';
                item = createMemoItem({
                    className: "asri-enhance-sidememo-blockmemo-item",
                    titleClassName: "asri-enhance-sidememo-blockmemo-item-title",
                    contentClassName: "asri-enhance-sidememo-blockmemo-item-content",
                    titleText,
                    contentText,
                    top,
                    uid,
                ...
                type = 'file';
                item = createMemoItem({
                    className: "asri-enhance-sidememo-filememo-item",
                    titleClassName: "asri-enhance-sidememo-filememo-item-title",
                    contentClassName: "asri-enhance-sidememo-filememo-item-content",
                    titleText,
                    contentText,
                    top,
                    uid,
    ...
    renderGraphvizInContainer(container);
    ...
    const toggleTooltipMemoNoneFor = (relatedEl: HTMLElement | null, add: boolean) => {
        try {
            ...
            if (add) {
                try { if (relatedEl.hasAttribute?.("asri-enhance-sidememo-fold")) return; } catch (e) {}
            }
    ...
    items.forEach((it) => {
        try {
            ...
            const onItemMouseDown = (startEvent: MouseEvent) => {
                ...
                const preMove = (moveEv: MouseEvent) => {
                    ...
                    if (Math.abs(moveEv.clientX - startEvent.clientX) > 6 || Math.abs(moveEv.clientY - startEvent.clientY) > 6) {
                        const h = (it.el as any).__asriSidememoHandlers;
                ...
                const timerId = window.setTimeout(() => {
                    ...
                    const asriParent = relatedEls[0]?.closest(".asri-enhance-sidememo") as HTMLElement | null;
                    if (!asriParent) return;
                        ...
                        try {
                            ...
                            if (_rafId === null) {
                                _rafId = requestAnimationFrame(() => {
                                    try { if (_pendingWidth !== null) asriParent.style.setProperty("--asri-enhance-sidememo-container-width", `${_pendingWidth}px`); } catch (e) {}
                                    _rafId = null; _pendingWidth = null;
                                });
                            }
                        } catch (e) {}
                    };
                    const dragUp = () => {
                        ...
                        try { if (_rafId !== null) { cancelAnimationFrame(_rafId); _rafId = null; _pendingWidth = null; } } catch (e) {}
            ...
            try {
                ...
                if (titleEl) {
                    const onTitleLeftClick = (ev: MouseEvent) => {
                        ...
                        try {
                            ...
                            if (firstRelated?.hasAttribute?.("memo")) {
                                ...
                                try { (targetEl ?? firstRelated).dispatchEvent(clickEvent); } catch (e) {}
                                ...
                                try { firstRelated.dispatchEvent(ctxEvent); } catch (e) {}
                            }
                        } catch (e) {}
                    };
                    ...
                    const onTitleRightClick = (ev: MouseEvent) => {
                        ...
                        try { syncSidememoState(); refreshSidememoContainers(); } catch (e) {}
                    };
                    ...
                    const merged = Object.assign(prev, {
                        ...
                        click: onTitleLeftClick, titleRightClick: onTitleRightClick,
                    ...
                    const prev = (it.el as any).__asriSidememoHandlers || {};
                    const merged = Object.assign(prev, { enter: onItemEnter, leave: onItemLeave, mousedown: onItemMouseDown });
                    (it.el as any).__asriSidememoHandlers = merged;
                    it.el.addEventListener("mousedown", onItemMouseDown);
                }
            } catch (e) {
            ...
            try {
                relatedEls.forEach(related => {
                    (related as any).__asriSidememoHandlers = { enter: onMemoEnter, leave: onMemoLeave };
                });
            } catch (e) {}
        } catch (e) {}
    });
...
export function removeAllSidememoArtifacts(): void {
    try {
        ...
        const containers = Array.from(document.querySelectorAll<HTMLElement>(".asri-enhance-sidememo-container"));
                    ...
                    try {
                        ...
                        if (handlers) {
                            ...
                            if (handlers.titleRightClick) {
                                try {
                                    const titleEl = child.querySelector(".asri-enhance-sidememo-inlinememo-item-title, .asri-enhance-sidememo-blockmemo-item-title, .asri-enhance-sidememo-filememo-item-title") as HTMLElement | null;
                                    if (titleEl) titleEl.removeEventListener("contextmenu", handlers.titleRightClick);
                                } catch (e) {}
                            }
                            if (handlers._timer) clearTimeout(handlers._timer);
                            try { if (handlers._asriContainer) handlers._asriContainer.classList.remove("asri-enhance-sidememo-container-resize"); } catch (e) {}
                            try { if (handlers._asriParent) handlers._asriParent.classList.remove("asri-enhance-sidememo-container-resize"); } catch (e) {}
                            delete (child as any).__asriSidememoHandlers;
                        }
                    } catch (e) {}
                });
        ...
        protyles.forEach((el) => {
            try {
                el.classList.remove("asri-enhance-sidememo", "asri-enhance-sidememo-none");
                ...
                memos.forEach((m) => {
                    try {
                        ...
                        try { m.removeAttribute("asri-enhance-sidememo-highlight"); } catch (e) {}
                    } catch (e) {}
                });
            } catch (e) {}
        });
        document.querySelectorAll(".asri-enhance-sidememo-container-resize").forEach(el => {
            try { (el as HTMLElement).classList.remove("asri-enhance-sidememo-container-resize"); } catch (e) {}
        });
        document.querySelectorAll(".protyle-title").forEach(el => {
            try { (el as HTMLElement).style.removeProperty("--asri-enhance-sidememo-title-height"); } catch (e) {}
        });
        document.querySelectorAll(".protyle-top").forEach(el => {
            try { (el as HTMLElement).classList.remove("asri-enhance-sidememo-protyle-title-none"); } catch (e) {}
        });
        document.querySelectorAll(".asri-enhance-sidememo").forEach(el => {
            try {
                (el as HTMLElement).style.removeProperty("--asri-enhance-sidememo-container-width");
                (el as HTMLElement).classList.remove("asri-enhance-sidememo");
            } catch (e) {}
        });
        document.querySelectorAll('[data-type*="inline-memo"][asri-enhance-sidememo-fold], [memo][asri-enhance-sidememo-fold]').forEach(el => {
            try { (el as HTMLElement).removeAttribute("asri-enhance-sidememo-fold"); } catch (e) {}
        });
        document.querySelectorAll(".tooltip--memo#tooltip").forEach(el => {
            try { (el as HTMLElement).classList.remove("asri-enhance-sidememo-tooltip-memo-none"); } catch (e) {}
        });
    } catch (e) {}
}
async function startObserver(): Promise<void> {
    if (fetchInterceptorDisconnect) return;
    try {
        ...
        fetchInterceptorDisconnect = () => {
            try { interceptor.disconnect(); } catch (err) {}
...
export function stopObserver(): void {
    if (fetchInterceptorDisconnect) {
        try { fetchInterceptorDisconnect(); } catch (err) {}
        fetchInterceptorDisconnect = null;
    }
}
export async function onSideMemoClick(plugin: Plugin, event?: MouseEvent): Promise<void> {
    if (isMobile()) return;
