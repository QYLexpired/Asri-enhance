...
function ensureSidememoContainers(): void {
    const titleElements = Array.from(document.querySelectorAll<HTMLElement>(".asri-enhance-sidememo .protyle-top .protyle-title"));
    ...
    const allContainers = Array.from(document.querySelectorAll<HTMLElement>(".protyle-top .protyle-title > .asri-enhance-sidememo-container"));
...
function populateSidememoContainer(container: HTMLElement, protyleContent: HTMLElement): void {
    const existingMemoEls = Array.from(protyleContent.querySelectorAll<HTMLElement>('[data-type="inline-memo"], [memo]'));
        ...
        try {
            ...
            if (handlers) {
                ...
                if (handlers._preMove)
                    document.removeEventListener("mousemove", handlers._preMove);
                if (handlers._preUp)
                    document.removeEventListener("mouseup", handlers._preUp);
                if (handlers._dragMove)
                    document.removeEventListener("mousemove", handlers._dragMove);
                if (handlers._dragUp)
                    document.removeEventListener("mouseup", handlers._dragUp);
                if (handlers._timer)
                    clearTimeout(handlers._timer);
                try {
                    if (handlers._asriContainer) {
                        handlers._asriContainer.classList.remove("asri-enhance-sidememo-container-resize");
                    }
                    if (handlers._asriParent) {
                        handlers._asriParent.classList.remove("asri-enhance-sidememo-container-resize");
                    }
                }
                catch (e) { }
    ...
    const memoElements = Array.from(protyleContent.querySelectorAll<HTMLElement>('[data-type="inline-memo"], [memo]'));
    type ItemWithTop = { el: HTMLElement; top: number; height: number; uid?: string; sourceEl?: HTMLElement };
    const items: ItemWithTop[] = [];
    memoElements.forEach((memoEl) => {
        try {
            if (memoEl.hasAttribute("data-type") && memoEl.getAttribute("data-type") === "inline-memo") {
                const titleText = (memoEl.textContent || "").trim();
                const contentText = memoEl.getAttribute("data-inline-memo-content") || "";
                ...
                content.className = "asri-enhance-sidememo-inlinememo-item-content";
                try {
                    const mdHtml = (typeof marked === "function"
                        ? marked(contentText)
                        : (marked && (marked.parse || marked.default)
                            ? (marked.parse ? marked.parse(contentText) : (marked.default ? marked.default(contentText) : contentText))
                            : contentText));
                ...
                title.textContent = titleText;
                const content = document.createElement("div");
                content.className = "asri-enhance-sidememo-blockmemo-item-content";
                try {
                    const mdHtml = (typeof marked === "function"
                        ? marked(contentText)
                        : (marked && (marked.parse || marked.default)
    ...
    items.forEach((it) => {
        try {
            ...
            if (!related)
                return;
            const onItemEnter = () => related.classList.add("asri-enhance-sidememo-highlight");
            const onItemLeave = () => related.classList.remove("asri-enhance-sidememo-highlight");
            const onMemoEnter = () => it.el.classList.add("asri-enhance-sidememo-highlight");
            const onMemoLeave = () => it.el.classList.remove("asri-enhance-sidememo-highlight");
            ...
            const onItemMouseDown = (startEvent: MouseEvent) => {
                ...
                const preMove = (moveEv: MouseEvent) => {
                    ...
                    if (Math.abs(moveEv.clientX - startEvent.clientX) > 6 || Math.abs(moveEv.clientY - startEvent.clientY) > 6) {
                        if ((it.el as any).__asriSidememoHandlers && (it.el as any).__asriSidememoHandlers._timer) {
                            clearTimeout((it.el as any).__asriSidememoHandlers._timer);
                ...
                const timerId = window.setTimeout(() => {
                    ...
                    const asriParent = related.closest(".asri-enhance-sidememo") as HTMLElement | null;
                    if (!asriParent)
                        return;
                    const containerEl = container;
                    try {
                        containerEl.classList.add("asri-enhance-sidememo-container-resize");
                    }
                    catch (e) { }
                    const widthStr = getComputedStyle(asriParent).getPropertyValue("--asri-enhance-sidememo-container-width") || "";
                    const initialWidth = Math.max(100, Math.min(500, Math.round(parseFloat(widthStr) || 200)));
                    let dragMove: ((ev: MouseEvent) => void) | null = null;
                    let dragUp: (() => void) | null = null;
                    dragMove = (mv: MouseEvent) => {
                        try {
                            const newWidth = Math.max(100, Math.min(500, Math.round(initialWidth + (startX - mv.clientX))));
                            asriParent.style.setProperty("--asri-enhance-sidememo-container-width", `${newWidth}px`);
                        }
                        catch (e) { }
                    };
                    dragUp = () => {
            ...
            try {
                const titleEl = it.el.querySelector<HTMLElement>(".asri-enhance-sidememo-inlinememo-item-title, .asri-enhance-sidememo-blockmemo-item-title");
                if (titleEl) {
                    const onTitleClick = (ev: MouseEvent) => {
                        ...
                        try {
                            if (related.hasAttribute && related.hasAttribute("memo")) {
                                let targetEl: HTMLElement | null = null;
                                try {
                                    targetEl = related.querySelector<HTMLElement>(".protyle-attr .protyle-attr--memo");
                                }
                                catch (e) {
                                    targetEl = null;
                                }
                                const rect = (targetEl ?? related).getBoundingClientRect();
                                ...
                                const targetRect = related.getBoundingClientRect();
                    ...
                    const merged = Object.assign(prev, { enter: onItemEnter, leave: onItemLeave, click: onTitleClick, mousedown: onItemMouseDown });
...
export function removeAllSidememoArtifacts(): void {
    try {
                    ...
                    try {
                        ...
                        if (handlers) {
                            ...
                            if (handlers._timer)
                                clearTimeout(handlers._timer);
                            if (handlers._asriContainer)
                                try { handlers._asriContainer.classList.remove("asri-enhance-sidememo-container-resize"); } catch (e) { }
                            if (handlers._asriParent)
                                try { handlers._asriParent.classList.remove("asri-enhance-sidememo-container-resize"); } catch (e) { }
                            delete (child as any).__asriSidememoHandlers;
                        }
                    }
                    catch (e) { }
            ...
            try {
                ...
                memos.forEach((m) => {
                    try {
                        ...
                        try {
                            m.classList.remove("asri-enhance-sidememo-highlight");
                        }
                        catch (e) { }
                    }
                    catch (e) { }
        ...
        const remaining = Array.from(document.querySelectorAll<HTMLElement>(".asri-enhance-sidememo-container-resize"));
...
async function startObserver(): Promise<void> {
    ...
    try {
        const interceptor = createFetchInterceptor(/setUILayout|transactions|setBlockAttrs/, (_url?: string) => {
            try {
                setTimeout(() => {
                    try {
