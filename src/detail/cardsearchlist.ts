...
const CONFIG_KEY = "asri-enhance-cardsearchlist";
const DATA_ATTR = "data-asri-enhance-cardsearchlist";
export async function onCardSearchListClick(plugin: Plugin, event?: MouseEvent): Promise<void> {
    if (event) {
...
export async function applyCardSearchListConfig(plugin: Plugin, config?: Record<string, any> | null): Promise<void> {
    const htmlEl = document.documentElement;
    if (!htmlEl) {
        return;
    }
