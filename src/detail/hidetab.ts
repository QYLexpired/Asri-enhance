...
export async function onHideTabClick(plugin: Plugin, event?: MouseEvent): Promise<void> {
    ...
    const isActive = htmlEl.hasAttribute("data-asri-enhance-hide-tab");
    const config = await loadData(plugin, CONFIG_FILE) || {};
    if (isActive) {
        htmlEl.removeAttribute("data-asri-enhance-hide-tab");
        config[CONFIG_KEY] = false;
    }
    else {
        htmlEl.setAttribute("data-asri-enhance-hide-tab", "true");
