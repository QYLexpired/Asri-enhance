...
import { removeAmberConfig, removeWildernessConfig, removeMidnightConfig, clearAllPluginConfig } from "./utils/storage";
class AsriEnhancePlugin extends Plugin {
	...
	async onload() {
		...
		this.asriConfigClickHandler = (event: MouseEvent) => {
			...
			if (!target) return;
			if (target.closest("#asri-enhance-amber") || target.closest("#asri-enhance-wilderness") || target.closest("#asri-enhance-midnight")) {
				return;
			}
