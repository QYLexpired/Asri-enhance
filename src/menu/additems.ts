...
export function listenBarModeClick(plugin: Plugin, callback: (event: MouseEvent) => void, delayMs: number = 0): Unsubscribe {
...
export function addMoreAfterTopbarFusionPlus(plugin: Plugin, delayMs: number = 0): Unsubscribe {
    let timeoutId: number | undefined;
    let pollTimerId: number | undefined;
    let boundTarget: HTMLElement | null = null;
    let attempts = 0;
                ...
                if (topbarFusionPlus) {
                    ...
                    if (parent) {
                        ...
                        if (immersiveItem) {
                            ...
                            if (immersiveSubmenu && !immersiveSubmenu.querySelector("#asri-enhance-typewriter")) {
                                const typewriterButton = document.createElement("button");
                                typewriterButton.className = "b3-menu__item";
                                typewriterButton.id = "asri-enhance-typewriter";
                                typewriterButton.innerHTML = `${MORE_ICON_SVG}<span class="b3-menu__label">${plugin.i18n?.typewriter || "typewriterMode"}</span>`;
                                immersiveSubmenu.appendChild(typewriterButton);
                            }
                            if (immersiveSubmenu && !immersiveSubmenu.querySelector("#asri-enhance-focus")) {
                                const focusModeButton = document.createElement("button");
                                focusModeButton.className = "b3-menu__item";
                                focusModeButton.id = "asri-enhance-focus";
                                focusModeButton.innerHTML = `${MORE_ICON_SVG}<span class="b3-menu__label">${plugin.i18n?.focus || "focusMode"}</span>`;
                                immersiveSubmenu.appendChild(focusModeButton);
                            }
                            const typewriterItem = immersiveSubmenu?.querySelector<HTMLButtonElement>("#asri-enhance-typewriter");
                            if (typewriterItem) {
                                typewriterItem.onclick = (event) => onTypewriterModeClick(plugin, event);
                            }
                            const focusModeItem = immersiveSubmenu?.querySelector<HTMLButtonElement>("#asri-enhance-focus");
                            if (focusModeItem) {
                                focusModeItem.onclick = (event) => onFocusModeClick(plugin, event);
                            }
                        }
                    }
                    return;
